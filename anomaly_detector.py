...
RESULTS_DIR = 'ads/results'
MODEL_PATH = os.path.join(RESULTS_DIR, 'saved_transformer_model.h5')
SCALER_PATH = os.path.join(RESULTS_DIR, 'scaler.joblib')
ENCODER_PATH = os.path.join(RESULTS_DIR, 'encoder.joblib')
TARGET_ENCODER_PATH = os.path.join(RESULTS_DIR, 'target_encoder.joblib')
...
CATEGORICAL_COLS_FOR_ENCODER = [
    'proto', 'service', 'conn_state', 'dns_query', 'ssl_version', 'ssl_cipher', 'ssl_subject', 'ssl_issuer', 'http_method', 'http_uri', 'http_referrer', 'http_version', 'http_user_agent', 'http_orig_mime_types', 'http_resp_mime_types', 'weird_name', 'weird_addl'
    ]

NUMERICAL_COLS_FOR_SCALER = [
    'src_port', 'dst_port', 'duration', 'src_bytes', 'dst_bytes', 'missed_bytes', 'src_pkts', 'src_ip_bytes', 'dst_pkts', 'dst_ip_bytes', 'dns_qclass', 'dns_qtype', 'dns_rcode', 'dns_AA', 'dns_RD', 'dns_RA', 'dns_rejected', 'ssl_resumed', 'ssl_established', 'http_trans_depth', 'http_request_body_len', 'http_response_body_len', 'http_status_code', 'weird_notice', 'hour', 'day', 'day_of_week', 'src_ip_parsed', 'dst_ip_parsed'
    ]
...
def preprocess_data(df):
    ...
    try:
        ...
        all_expected_features = NUMERICAL_COLS_FOR_SCALER + CATEGORICAL_COLS_FOR_ENCODER
        for col in all_expected_features:
            if col not in df.columns:
                if col in CATEGORICAL_COLS_FOR_ENCODER:
                    ...
                    df[col] = 0
        X_num_features = df[NUMERICAL_COLS_FOR_SCALER]
        X_cat_features = df[CATEGORICAL_COLS_FOR_ENCODER]
        ...
        # Ensure string types before encoding
        X_cat_encoded = encoder.transform(X_cat_features.astype(str))
        ...
        # print(f"Preprocessing complete. Output shape: {X_reshaped.shape}") # Optional debug
        return X_reshaped
        ...
        try:
            ...
            if new_lines_content:
                ...
                try:
                    ...
                    new_df = pd.read_csv(StringIO(new_data_str),
                                     names=LOG_FILE_COLUMNS,
                                     header=None, # Correct for reading chunks without header
                ...
                if new_df is None or new_df.empty:
                    print("WARNING: Log chunk resulted in empty DataFrame after parsing. Skipping.")
                    processed_rows = current_row_count
                    continue
